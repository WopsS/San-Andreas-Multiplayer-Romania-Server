...
enum class ObjectEditionResponse : uint16_t
{
...
namespace Object
{
	...
	const uint32_t Create(uint32_t ModelID, const Point3D<float>& Position, const Point3D<float>& Rotation, int VirtualWorld = -1, int Interior = -1, int PlayerID = -1, float StreamDistance = STREAMER_OBJECT_SD, float DrawDistance = STREAMER_OBJECT_DD);
