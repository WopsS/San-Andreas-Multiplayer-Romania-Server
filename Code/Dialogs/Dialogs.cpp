...
void Dialogs::Register(std::shared_ptr<Dialog> Dialog)
{
	m_dialogs.emplace(Dialog->GetID(), Dialog);
}

CustomDialog Dialogs::Create(const DialogID& ID, const std::string& Text)
{
	auto Dialog = Get(ID);

	auto Buttons = Dialog->GetButtons();
	std::string Result[2] = { Buttons[0], Buttons[1] };

	return Create(ID, Dialog->GetStyle(), Dialog->GetCaption(), Text, Result);
}

CustomDialog Dialogs::Create(const DialogID& ID, const DialogStyle& Style, const std::string& Caption, const std::string& Text, const std::string Buttons[2])
{
	return CustomDialog(ID, Style, Caption, Text, Buttons);
}
...
bool Dialogs::Execute(std::shared_ptr<Player> Player, const DialogID& ID, const DialogResponse& Response, const size_t ItemIndex, const std::string& Text)
{
	...
	if (Dialog->CheckAccess(Player) == true)
	{
		Dialog->Execute(Player, Response, ItemIndex, Text);
	}
